...
app.get("/recados", function (requisicao, resposta) {
  ...
  const titulo = requisicao.query.titulo;
  if (titulo) {
    const recadosFiltrados = recados.filter(function (recado) {
      return recado.titulo.includes(titulo);
    });
    resposta.json({
...
app.delete("/recados/:id/deletar", function (requisicao, resposta) {
  const id = parseInt(requisicao.params.id);
